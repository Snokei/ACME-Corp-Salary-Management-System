...
function getEmployeeAvatar(emp: Employee): string {
  if (
    emp.avatarUrl &&
    emp.avatarUrl.startsWith("http") &&
    !emp.avatarUrl.includes("photo-NaN")
  ) {
...
export function EmployeeDetailDrawer({
  ...
  const [compAnalysis, setCompAnalysis] =
    useState<CompensationAnalysisResult | null>(null);
  ...
  useEffect(() => {
    if (currentEmp) {
      ...
      getCompensationAnalysisAction(
        salary,
        currentEmp.payGrade || "L4",
        currentEmp.currency || "USD",
      )
        .then((res) => {
    ...
    currentEmp?.id,
    currentEmp?.baseSalary,
    currentEmp?.baseSalaryUSD,
    currentEmp?.payGrade,
  ]);
  ...
  const currentSalaryAmount =
    currentEmp.baseSalaryUSD ?? currentEmp.baseSalary ?? 0;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 py-1 text-xs">
            ...
            <div className="col-span-2 p-3 rounded-2xl bg-stone-50 dark:bg-stone-800/60 border border-stone-100 dark:border-stone-800">
              ...
              <span className="font-semibold text-stone-800 dark:text-stone-100 mt-0.5 block">
                {currentEmp.city
                  ? `${currentEmp.city}, ${currentEmp.country}`
                  : currentEmp.country}
              </span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-stone-50 dark:bg-stone-800/60 border border-stone-200 dark:border-stone-700/80 space-y-3">
            ...
            {loadingAnalysis ? (
              ...
              <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-200 text-xs flex items-center gap-2">
                ...
                <span>
                  No matching salary band defined for pay grade (
                  {currentEmp.payGrade || "N/A"}).
                </span>
              </div>
            ) : (
              <div className="space-y-3 text-xs">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
                  ...
                  <div className="p-2 rounded-xl bg-white/80 dark:bg-stone-900/60 border border-amber-400/40">
                    ...
                    <span className="font-extrabold text-stone-900 dark:text-white text-sm">
                      {compAnalysis.compaRatio !== null
                        ? `${compAnalysis.compaRatio.toFixed(1)}%`
                        : "N/A"}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 pt-1">
                  <div className="flex items-center justify-between text-[10px] font-semibold text-stone-500 dark:text-stone-400">
                    <span>
                      ...
                      <strong className="text-stone-900 dark:text-white">
                        {compAnalysis.displayPosition}
                      </strong>
                    </span>
                    <span>
                      Grade {compAnalysis.band.payGrade} (
                      {compAnalysis.band.currency})
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between text-[9px] text-stone-400 font-mono">
                    ...
                    <span>
                      Mid: ${compAnalysis.band.midpointSalary.toLocaleString()}
                    </span>
                ...
                {salaryHistory.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[11px] text-stone-500 dark:text-stone-400">
                      ...
                      {item.createdBy && (
                        <span className="text-[10px]">By {item.createdBy}</span>
                      )}
                    </div>
