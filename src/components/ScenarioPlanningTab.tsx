...
import { Sparkles, TrendingUp, DollarSign, Users, Percent, PieChart, CheckCircle2, ArrowRight } from "lucide-react";
...
export function ScenarioPlanningTab({
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="p-6 rounded-3xl bg-white dark:bg-stone-900/80 border border-stone-200/80 dark:border-stone-800/80 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 bg-stone-50 dark:bg-stone-950 p-2 rounded-2xl border border-stone-200 dark:border-stone-800">
          ...
          <div className="relative w-24">
            <input
              ...
              className="w-full pr-5 pl-2 py-1 rounded-xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 text-right text-xs font-mono font-bold text-stone-900 dark:text-white focus:ring-2 focus:ring-amber-500/50"
            />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {scenarios.map((scen) => {
          ...
          return (
            <div
              key={scen.id}
              className={`p-6 rounded-3xl bg-white dark:bg-stone-900/80 border transition-all flex flex-col justify-between space-y-6 ${
                isOverBudget
                  ? "border-rose-500/30 hover:border-rose-500/50"
                  : "border-stone-200/80 dark:border-stone-800/80 hover:border-amber-500/40"
            ...
            >
              <div>
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-mono font-extrabold text-stone-900 dark:text-white text-sm">
              ...
              {isEditable && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-2xl bg-stone-900 hover:bg-stone-800 dark:bg-stone-800 dark:hover:bg-amber-500 dark:hover:text-stone-950 text-white font-semibold text-xs transition-all disabled:opacity-40"
                >
                  {isApplying === scen.name ? (
