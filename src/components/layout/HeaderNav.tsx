...
export function HeaderNav() {
  ...
  // Committed profile — only updates after a successful save.
  const [profile, setProfile] = useState({
    name: CURRENT_USER.name,
    email: "admin@acme.com",
    role: CURRENT_USER.role,
  });
  // Draft bound to the edit form (includes optional password).
  ...
  const startEditing = () => {
    setDraft({ name: profile.name, email: profile.email, password: "" });
    setUi((prev) => ({ ...prev, isEditing: true }));
  };
  ...
  return (
    <header className="w-full flex items-center justify-between gap-4 py-3 px-0 relative z-50">
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
        ...
        >
          {ui.isMobileMenuOpen ? <X className="w-4 h-4" /> : <Menu className="w-4 h-4" />}
        </button>
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 top-full mt-2 w-48 rounded-2xl bg-white/90 dark:bg-stone-900/90 backdrop-blur-md border border-stone-200/50 dark:border-stone-800/50 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-2 group-hover:translate-y-0 z-50">
            <div className="p-1">
              <button
                onClick={() => setUi((prev) => ({ ...prev, isProfileOpen: true }))}
      ...
      <Modal
        isOpen={ui.isProfileOpen}
        onClose={() => {
          setUi((prev) => ({ ...prev, isProfileOpen: false }));
          cancelEditing();
        }}
        maxWidth="sm"
      >
        <div className="-mt-5 sm:-mt-6 -mx-5 sm:-mx-6 relative">
          ...
          <div className="px-4 sm:px-6 pb-6">
            ...
            <div className="mb-6">
              <h3 className="font-bold text-xl text-stone-900 dark:text-white tracking-tight break-words">
                {profile.name}
              </h3>
              <p className="text-sm font-medium text-amber-600 dark:text-amber-500">{profile.role}</p>
            </div>
